<?php
/**
 * Template part for displaying page content in page.php.
 *
 * @link https://codex.wordpress.org/Template_Hierarchy
 *
 * @package radiance
 */

?>
<?php the_content(); ?>
<!-- end html_open.html-->
<div class="container">
	<!-- start left_section.html-->
	<section class="left_section">
		<div class="logo_container">
			<a href="#" class="logo">
				<img src="<?php bloginfo('template_directory'); ?>/img/logo.png" alt="logo">
				<span class="line"></span>
				<span class="logo-dot"></span>
				<span>PREMIUM QUALITY CLOTHES</span>
			</a>

		</div>
		<div class="main_menu">
			<ul>
				<li><a class="main_menu__item" href="#"><span class="mnu_arrow"></span>NEW ARRIVALS</a></li>
				<li><a class="main_menu__item" href="#"><span class="mnu_arrow"></span>BEST SELLING</a></li>
				<li><a class="main_menu__item" href="#"><span class="mnu_arrow"></span>SALES & SPECIALS</a></li>
				<li><a class="main_menu__item" href="#"><span class="mnu_arrow"></span>MY ACCOUNT</a></li>
				<li><a class="main_menu__item" href="#"><span class="mnu_arrow"></span>CONTACT US</a></li>
			</ul>
		</div>
		<div class="catalogue">
			<h3>CATALOGUE</h3>
			<span class="catalogue__line"></span>
			<div class="visible">
				<ul class="catalogue_menu">
					<li><a class="catalogue_menu__item" href="#">CATALOG ITEM 1</a></li>
					<li><a class="catalogue_menu__item" href="#">CATALOG ITEM 2 NAME TITLE</a></li>
					<li><a class="catalogue_menu__item" href="#">CATALOG ITEM 3 NAME</a></li>
					<li><a class="catalogue_menu__item" href="#">CATALOG ITEM 4</a></li>
					<li><a class="catalogue_menu__item" href="#">CATALOG ITEM 5 TITLE</a></li>
					<li><a class="catalogue_menu__item" href="#">CATALOG ITEM 6</a></li>
					<li><a class="catalogue_menu__item" href="#">CATALOG ITEM 1</a></li>
					<li><a class="catalogue_menu__item" href="#">CATALOG ITEM 2 NAME TITLE</a></li>
					<li><a class="catalogue_menu__item" href="#">CATALOG ITEM 3 NAME</a></li>
					<li><a class="catalogue_menu__item" href="#">CATALOG ITEM 4</a></li>
					<li><a class="catalogue_menu__item" href="#">CATALOG ITEM 5 TITLE</a></li>
					<li><a class="catalogue_menu__item" href="#">CATALOG ITEM 6</a></li>
				</ul>
			</div>
			<a href="#" class="catalogue_arrow_up"></a>
			<a href="#" class="catalogue_arrow_down"></a>
		</div>
		<div class="custom_care">

			<h3>CUSTOMER CARE</h3>
			<span class="custom_care__line"></span>
			<ul class="custom_care__menu">
				<li><a class="custom_care__menu__item" href="#"><span class="custom_care__arrow"></span>TERMS & CONDITIONS</a></li>
				<li><a class="custom_care__menu__item" href="#"><span class="custom_care__arrow"></span>SHIPPING & PAYMENT INFORMATION</a></li>
				<li><a class="custom_care__menu__item" href="#"><span class="custom_care__arrow"></span>RETURNS POLICY</a></li>
				<li><a class="custom_care__menu__item" href="#"><span class="custom_care__arrow"></span>HOW TO TAKE CARE OF YOUR
						CLOTHES</a></li>
				<li><a class="custom_care__menu__item" href="#"><span class="custom_care__arrow"></span>TERMS & CONDITIONS</a></li>
				<li><a class="custom_care__menu__item" href="#"><span class="custom_care__arrow"></span>SHIPPING & PAYMENT INFORMATION</a></li>
				<li><a class="custom_care__menu__item" href="#"><span class="custom_care__arrow"></span>RETURNS POLICY</a></li>
				<li><a class="custom_care__menu__item" href="#"><span class="custom_care__arrow"></span>HOW TO TAKE CARE OF YOUR
						CLOTHES</a></li>

			</ul>
			<a href="#" class="custom_care__arrow_up"></a>
			<a href="#" class="custom_care__arrow_down"></a>
		</div>
		<footer class="foot-left-side">
			<div class="left-footer">
				<h4>WE ARE IN SOCIAL NETWORKS:</h4>
				<ul class="social">
					<li><a href="#" class="social_link"><span class="social_vk"></span></a></li>
					<li><a href="#" class="social_link"><span class="social_fb"></span></a></li>
					<li><a href="#" class="social_link"><span class="social_ok"></span></a></li>
					<li><a href="#" class="social_link"><span class="social_twitter"></span></a></li>
					<li><a href="#" class="social_link"><span class="social_googleplus"></span></a></li>
				</ul>
			</div>
		</footer>
	</section>
	<!-- end left_section.html-->
	<!-- start right_section.html-->
	<section class="right_section">

		<div class="slider-top">
			<div class="item">
				<div class="slide-item-big">
					<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-big.jpg" alt="slide-1">
					<div class="slider-text-big">
						<h4>lorem iosum</h4>
						<p>
							hi title test
						</p>
						<p>
							main block
						</p>
					</div>
				</div>
			</div>

			<div class="item">
				<div class="slide-item-big">
					<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-1.jpg" alt="slide-1">
					<div class="slider-text-big">
						<h4>lorem iosum</h4>
						<p>
							hi title test
						</p>
						<p>
							main block
						</p>
					</div>
				</div>
			</div>
			<div class="item">
				<div class="slide-item-big">
					<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-2.jpg"  alt="slide-1">
					<div class="slider-text-big">
						<h4>lorem iosum</h4>
						<p>
							hi title test
						</p>
						<p>
							main block
						</p>
					</div>
				</div>
			</div>
			<div class="item">
				<div class="slide-item-big">
					<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-3.jpg" alt="slide-1">
					<div class="slider-text-big">
						<h4>lorem iosum</h4>
						<p>
							hi title test
						</p>
						<p>
							main block
						</p>
					</div>
				</div>
			</div>
			<div class="item">
				<div class="slide-item-big">
					<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-big.jpg" alt="slide-1">
					<div class="slider-text-big">
						<h4 class="slide-header-big">lorem iosum</h4>
						<p>
							hi title test
						</p>
						<p>
							main block
						</p>
					</div>
				</div>
			</div>

			<div class="item">
				<div class="slide-item-big">
					<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-1.jpg" alt="slide-1">
					<div class="slider-text-big">
						<h4>lorem iosum</h4>
						<p>
							hi title test
						</p>
						<p>
							main block
						</p>
					</div>
				</div>
			</div>
			<div class="item">
				<div class="slide-item-big">
					<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-2.jpg"  alt="slide-1">
					<div class="slider-text-big">
						<h4>lorem iosum</h4>
						<p>
							hi title test
						</p>
						<p>
							main block
						</p>
					</div>
				</div>
			</div>
			<div class="item">
				<div class="slide-item-big">
					<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-3.jpg"  alt="slide-1">
					<div class="slider-text-big">
						<h4>lorem iosum</h4>
						<p>
							hi title test
						</p>
						<p>
							main block
						</p>
					</div>
				</div>
			</div>
		</div>

		<div class="slick-slider">
			<div class="slide-item">
				<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-big.jpg" alt="">
				<div class="slider-text-big">
					<h4>lorem iosum</h4>
					<p>
						hi title test
					</p>
					<p>
						main block
					</p>
				</div>
			</div>
			<div class="slide-item">
				<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-1.jpg" alt="">
				<div class="slider-text-big">
					<h4>lorem iosum</h4>
					<p>
						hi title test
					</p>
				</div>
			</div>
			<div class="slide-item">
				<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-2.jpg" alt="">
				<div class="slider-text-big">
					<h4>lorem iosum</h4>
					<p>
						hi title test
					</p>
				</div>
			</div>

			<div class="slide-item">
				<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-3.jpg" alt="">
				<div class="slider-text-big">
					<h4>lorem iosum</h4>
					<p>
						hi title test
					</p>
					<p>
						main block
					</p>
				</div>
			</div>
			<div class="slide-item">
				<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-big.jpg" alt="">
				<div class="slider-text-big">
					<h4>lorem iosum</h4>
					<p>
						hi title test
					</p>
				</div>
			</div>
			<div class="slide-item">
				<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-1.jpg" alt="">
				<div class="slider-text-big">
					<h4>lorem iosum</h4>
					<p>
						hi title test
					</p>
				</div>
			</div>
			<div class="slide-item">
				<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-2.jpg" alt="">
				<div class="slider-text-big">
					<h4>lorem iosum</h4>
					<p>
						hi title test
					</p>
				</div>
			</div>
			<div class="slide-item">
				<img src="<?php bloginfo('template_directory'); ?>/img/slider/slide-item-3.jpg" alt="">
				<div class="slider-text-big">
					<h4>lorem iosum</h4>
					<p>
						hi title test
					</p>
				</div>
			</div>


		</div>

		<div class="panel">
			<div class="currencies">
				<p>currencies:</p>
				<a class="currency active-currency" href="#">USD</a>
				<a class="currency" href="#">EUR</a>
				<a class="currency" href="#">RUR</a>
				<span class="line"></span>
			</div>

			<div class="search-container">
				<p class="search-container-header">search:</p>
				<form role="search" method="get" class="search search-form" action="">
					<input class="search-field" type="search" placeholder="" name="s">
					<input type="submit" class="search-sbm" value="">
				</form>
				<span class="line"></span>
			</div>
			<div class="shoping-cart">
				<p class="shoping-cart-header">shoping cart:</p>
				<a href="#" class="panel-basket"></a>
				<div class="goods-usd">
					<p class="goods"><span>24</span> goods</p>
					<p class="usd"><span>235</span>  usd</p>
				</div>
				<span class="line"></span>
			</div>

			<div class="languages">
				<p class="languages-header">language:</p>
				<a class="language active-currency" href="#">EN</a>
				<a class="language" href="#">FR</a>
				<a class="language" href="#">RU</a>
				<span class="line"></span>
			</div>

		</div>


		<div class="owl-carousel slider-bot">
			<div class="item">
				<div class="product">

					<div class="product-image">
						<img src="<?php bloginfo('template_directory'); ?>/img/products/product_1.jpg" alt="">
					</div>
					<div class="product-info">
						<h3 class="product-header">PRODUCT TITLE TEST
							TITLE TEST PRODUCT 1</h3>
						<span class="line"></span>
						<span class="article">ARTICLE:<span class="article-value">#00009090234</span></span>
						<p class="product-descr">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
						<p class="price"><span class="price-icon">$</span>225,95</p>
						<a class="add-to-card" href="#"><span class="add-to-card-icon"></span>ADD
							TO CART</a>
					</div>
				</div>
				<div class="product">

					<div class="product-image">
						<img src="<?php bloginfo('template_directory'); ?>/img/products/product_1.jpg" alt="">
					</div>
					<div class="product-info">
						<h3 class="product-header">PRODUCT TITLE TEST
							TITLE TEST PRODUCT 1</h3>
						<span class="line"></span>
						<span class="article">ARTICLE:<span class="article-value">#00009090234</span></span>
						<p class="product-descr">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
						<p class="price"><span class="price-icon">$</span>225,95</p>
						<a class="add-to-card" href="#"><span class="add-to-card-icon"></span>ADD
							TO CART</a>
					</div>
				</div>
				<div class="product">

					<div class="product-image">
						<img src="<?php bloginfo('template_directory'); ?>/img/products/product_1.jpg" alt="">
					</div>
					<div class="product-info">
						<h3 class="product-header">PRODUCT TITLE TEST
							TITLE TEST PRODUCT 1</h3>
						<span class="line"></span>
						<span class="article">ARTICLE:<span class="article-value">#00009090234</span></span>
						<p class="product-descr">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
						<p class="price"><span class="price-icon">$</span>225,95</p>
						<a class="add-to-card" href="#"><span class="add-to-card-icon"></span>ADD
							TO CART</a>
					</div>
				</div>
				<div class="product">

					<div class="product-image">
						<img src="<?php bloginfo('template_directory'); ?>/img/products/product_1.jpg" alt="">
					</div>
					<div class="product-info">
						<h3 class="product-header">PRODUCT TITLE TEST
							TITLE TEST PRODUCT 1</h3>
						<span class="line"></span>
						<span class="article">ARTICLE:<span class="article-value">#00009090234</span></span>
						<p class="product-descr">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
						<p class="price"><span class="price-icon">$</span>225,95</p>
						<a class="add-to-card" href="#"><span class="add-to-card-icon"></span>ADD
							TO CART</a>
					</div>
				</div>
			</div>

			<div class="item">
				<div class="product">

					<div class="product-image">
						<img src="<?php bloginfo('template_directory'); ?>/img/products/product_1.jpg" alt="">
					</div>
					<div class="product-info">
						<h3 class="product-header">PRODUCT TITLE TEST
							TITLE TEST PRODUCT 1</h3>
						<span class="line"></span>
						<span class="article">ARTICLE:<span class="article-value">#00009090234</span></span>
						<p class="product-descr">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
						<p class="price"><span class="price-icon">$</span>225,95</p>
						<a class="add-to-card" href="#"><span class="add-to-card-icon"></span>ADD
							TO CART</a>
					</div>
				</div>
				<div class="product">

					<div class="product-image">
						<img src="<?php bloginfo('template_directory'); ?>/img/products/product_1.jpg" alt="">
					</div>
					<div class="product-info">
						<h3 class="product-header">PRODUCT TITLE TEST
							TITLE TEST PRODUCT 1</h3>
						<span class="line"></span>
						<span class="article">ARTICLE:<span class="article-value">#00009090234</span></span>
						<p class="product-descr">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
						<p class="price"><span class="price-icon">$</span>225,95</p>
						<a class="add-to-card" href="#"><span class="add-to-card-icon"></span>ADD
							TO CART</a>
					</div>
				</div>
				<div class="product">

					<div class="product-image">
						<img src="<?php bloginfo('template_directory'); ?>/img/products/product_1.jpg" alt="">
					</div>
					<div class="product-info">
						<h3 class="product-header">PRODUCT TITLE TEST
							TITLE TEST PRODUCT 1</h3>
						<span class="line"></span>
						<span class="article">ARTICLE:<span class="article-value">#00009090234</span></span>
						<p class="product-descr">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
						<p class="price"><span class="price-icon">$</span>225,95</p>
						<a class="add-to-card" href="#"><span class="add-to-card-icon"></span>ADD
							TO CART</a>
					</div>
				</div>
				<div class="product">

					<div class="product-image">
						<img src="<?php bloginfo('template_directory'); ?>/img/products/product_1.jpg" alt="">
					</div>
					<div class="product-info">
						<h3 class="product-header">PRODUCT TITLE TEST
							TITLE TEST PRODUCT 1</h3>
						<span class="line"></span>
						<span class="article">ARTICLE:<span class="article-value">#00009090234</span></span>
						<p class="product-descr">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
						<p class="price"><span class="price-icon">$</span>225,95</p>
						<a class="add-to-card" href="#"><span class="add-to-card-icon"></span>ADD
							TO CART</a>
					</div>
				</div>
			</div>

			<div class="item">
				<div class="product">

					<div class="product-image">
						<img src="<?php bloginfo('template_directory'); ?>/img/products/product_1.jpg" alt="">
					</div>
					<div class="product-info">
						<h3 class="product-header">PRODUCT TITLE TEST
							TITLE TEST PRODUCT 1</h3>
						<span class="line"></span>
						<span class="article">ARTICLE:<span class="article-value">#00009090234</span></span>
						<p class="product-descr">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
						<p class="price"><span class="price-icon">$</span>225,95</p>
						<a class="add-to-card" href="#"><span class="add-to-card-icon"></span>ADD
							TO CART</a>
					</div>
				</div>
				<div class="product">

					<div class="product-image">
						<img src="<?php bloginfo('template_directory'); ?>/img/products/product_1.jpg" alt="">
					</div>
					<div class="product-info">
						<h3 class="product-header">PRODUCT TITLE TEST
							TITLE TEST PRODUCT 1</h3>
						<span class="line"></span>
						<span class="article">ARTICLE:<span class="article-value">#00009090234</span></span>
						<p class="product-descr">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
						<p class="price"><span class="price-icon">$</span>225,95</p>
						<a class="add-to-card" href="#"><span class="add-to-card-icon"></span>ADD
							TO CART</a>
					</div>
				</div>
				<div class="product">

					<div class="product-image">
						<img src="<?php bloginfo('template_directory'); ?>/img/products/product_1.jpg" alt="">
					</div>
					<div class="product-info">
						<h3 class="product-header">PRODUCT TITLE TEST
							TITLE TEST PRODUCT 1</h3>
						<span class="line"></span>
						<span class="article">ARTICLE:<span class="article-value">#00009090234</span></span>
						<p class="product-descr">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
						<p class="price"><span class="price-icon">$</span>225,95</p>
						<a class="add-to-card" href="#"><span class="add-to-card-icon"></span>ADD
							TO CART</a>
					</div>
				</div>
				<div class="product">

					<div class="product-image">
						<img src="<?php bloginfo('template_directory'); ?>/img/products/product_1.jpg" alt="">
					</div>
					<div class="product-info">
						<h3 class="product-header">PRODUCT TITLE TEST
							TITLE TEST PRODUCT 1</h3>
						<span class="line"></span>
						<span class="article">ARTICLE:<span class="article-value">#00009090234</span></span>
						<p class="product-descr">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
						<p class="price"><span class="price-icon">$</span>225,95</p>
						<a class="add-to-card" href="#"><span class="add-to-card-icon"></span>ADD
							TO CART</a>
					</div>
				</div>
			</div>



		</div>

	</section>
	<footer class="foot-right-side">
		<div class="right-footer">
			<div class="footer-mnu">
				<ul>
					<li><a href="#">new arrivals</a></li>
					<li><a href="#">sales & specials</a></li>
					<li><a href="#">create account</a></li>
					<li><a href="#">customer login</a></li>
					<li><a href="#"> contact us</a></li>
				</ul>
			</div>
			<span class="line-brown"></span>
			<div class="bot-footer">
				<div class="bot-footer__left-side">
					<a href="#">RADIANCE.GLOBAL</a>
					<a href="#">TERMS OF USE</a>
				</div>
				<div class="bot-footer__right-side">
					<ul>
						<li>RADIANCE &trade;</li>
						<li>&copy; 2016</li>
						<li><a href="#"><span class="logo_small"></span></a></li>
					</ul>
				</div>
			</div>
		</div>
	</footer>
	<!-- end right_section.html-->
</div>